...
*   **Commits e Push**: AI agents must **NEVER** perform git commits or pushes automatically. After completing any work:
    ...
    2.  **Run `git status`** before committing to ensure no files are accidentally left behind.
    3.  Wait for **explicit user confirmation** before executing any `git add`, `git commit`, or `git push` commands.
    4.  This rule applies even when fixing CI errors or making "obvious" corrections.
